...
// tier 色資料化：把 TRIP.tierColors 注入 CSS 變數 --tN-fg/bg（styles.css :root 值為無 JS 後備）；她在設定改色後（pick-washoku）會再呼叫一次即時生效
function tintHex(hex, w){ const m=/^#([0-9a-f]{6})$/i.exec(hex||''); if(!m) return hex; const n=parseInt(m[1],16), r=n>>16&255, g=n>>8&255, b=n&255, f=c=>Math.round(c*(1-w)+255*w), h=c=>('0'+c.toString(16)).slice(-2); return '#'+h(f(r))+h(f(g))+h(f(b)); }
function applyTierColors(){ const tc=(typeof TRIP!=='undefined'&&TRIP&&TRIP.tierColors)||null; if(!tc) return; const s=document.documentElement.style; ['1','2','3','4'].forEach(n=>{ const v=tc['t'+n]; if(!v) return; s.setProperty('--t'+n+'-fg', v.fg); s.setProperty('--t'+n+'-bg', v.bg); }); }
applyTierColors();
...
const TAB_KEY=V2_ENV?'cnx-tab-v2':'cnx-tab';   // 目前分頁（per-device、不進雲端）：reload 不跳回總覽
...
function applyDb(db){
  ...
  CNXCore.applyWashokuPalette(TRIP);   // 雲端同步進來的也套一次和色（flag 防重複）；下方 setItem 持久
  applyTierColors();                   // 雲端同步進來的 tierColors 也重新注入 CSS 變數
...
function lodgingIcon(){ const c=(TRIP.categories||[]).find(x=>x.role==='lodging'); return esc((c&&c.icon)||'🏨'); }   // 住宿類別她設的 icon（不寫死 🏨，Vivian #1）
function hotelName(p){ const n=(p&&p.name)||''; return n.replace(/^住宿\s*/,'')||n; }   // 顯示飯店名時去掉冗餘「住宿」前綴（🏠 已表意，Vivian）
...
function toast(msg, opts){ const el=document.getElementById('toast'); clearTimeout(toastT);
  ...
  el.classList.add('on');
  toastT=setTimeout(()=>el.classList.remove('on'), undo?6000:3300);   // 停留（Vivian：8s 太久）：有復原鈕 6s、一般 3.3s
}
...
function initMap(){
  ...
  L.tileLayer('https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png',{maxZoom:19,subdomains:'abcd',attribution:'© OpenStreetMap © CARTO'}).addTo(map);   // CartoDB Voyager（壓彩度 saturate(.6) 在 CSS、冷罩見 .map-cool；定稿 docs/redesign-prototypes/map.html）
  const c=document.getElementById('map'); if(c&&!c.querySelector('.map-cool')){ const ov=document.createElement('div'); ov.className='map-cool'; c.appendChild(ov); }   // #7e8eae @.08 冷色罩：壓黃偏冷（pointer-events:none、不擋互動）
...
function renderMarkers(){
  ...
  places.filter(p=>(p.lat&&p.lng)&&passLib(p)).forEach(p=>{
    const icon=L.divIcon({className:'',html:`<div class="mk-pin"><span>${placeEmoji(p)}</span></div>`,iconSize:[26,26],iconAnchor:[13,24],popupAnchor:[0,-24]});   // 中性白釘 teardrop（不加任何顏色；定稿 map.html；Vivian：小一點 30→26）
    ...
    const ctx=p.area?regionLabel(p.area):'';   // 版一 popup＝換將中心語言（店名+tier／區域／🕐💰💬／📅排入·📍Maps·✎編輯）
    m.bindPopup(`<div class="pp"><div class="pp-nm">${placeEmoji(p)} ${esc(p.name)}${p.tier?tierBadge(p.tier):''}</div>${ctx?`<div class="pp-ctx">${esc(ctx)}</div>`:''}<div class="pp-info"><div>🕐 ${esc(CNXCore.condenseHours(p))||'—'}</div><div>💰 ${costLabel(p.cost)||'—'}</div>${memo?`<div>💬 ${esc(memo)}</div>`:''}</div><div class="pp-acts"><span class="pp-a pri" data-action="assign" data-id="${p.id}">📅 排入</span><a class="pp-a" href="${esc(gmaps(p))}" target="_blank" rel="noopener">📍 Maps</a><span class="pp-a" data-action="edit" data-id="${p.id}">✎ 編輯</span></div></div>`);
    markers.push(m);
  });
...
function autoSpot(ids, opts){
  ...
  if(!isDesktop()){
    ...
    if(drawerOpen()){ spotFromCard=true; document.body.classList.add('cardpeek'); mapSpotlightSet([...set]); }   // 地圖已開→進共視 A 上下分割（地圖上半／看卡下半實心拼滿、不透出）＋更新 pin（不開抽屜、不捲動）
    return;
  }
...
function exitCardPeek(){
  document.body.classList.remove('cardpeek');    // 共視只加 class、不動地圖開關（進共視前提＝地圖已開）→ 關卡＝地圖留原樣
}
function isDesktop(){ return window.matchMedia&&window.matchMedia('(min-width:1100px)').matches; }   // 桌機三欄（spec §4.12）
// ── 地圖可拖縮放（Vivian #2）：桌機拖右緣改寬 --mapW（中欄總覽自動橫滑、格子不縮）／手機拖下緣改高 --mapH（總覽往下推、不壓縮）。預設值不變、純 inline 覆寫；不持久化 ──
(function(){
  if(!drawer) return;
  const bar=document.createElement('div'); bar.id='mapresize'; bar.setAttribute('aria-hidden','true'); drawer.appendChild(bar);
  const clamp=(v,lo,hi)=>Math.max(lo,Math.min(hi,v));
  let on=false, vert=false, p0=0, s0=0, raf=0;
  bar.addEventListener('pointerdown',ev=>{ if(!drawerOpen()) return;
    vert=!isDesktop(); on=true; bar.setPointerCapture(ev.pointerId); ev.preventDefault();
    if(vert){ p0=ev.clientY; const mb=drawer.querySelector('.mapbody'); s0=mb?mb.getBoundingClientRect().height:0; }
    else { p0=ev.clientX; s0=drawer.getBoundingClientRect().width; }
  });
  bar.addEventListener('pointermove',ev=>{ if(!on) return;
    if(vert){ const h=clamp(s0+(ev.clientY-p0),120,Math.round(window.innerHeight*0.78)); document.body.style.setProperty('--mapH',h+'px'); }
    else { const w=clamp(s0+(ev.clientX-p0),240,Math.round(window.innerWidth*0.70)); document.body.style.setProperty('--mapW',w+'px'); }
    if(!raf) raf=requestAnimationFrame(()=>{ raf=0; if(map) map.invalidateSize(); });
  });
  const end=ev=>{ if(!on) return; on=false; try{bar.releasePointerCapture(ev.pointerId);}catch(e){} if(map) map.invalidateSize(); };
