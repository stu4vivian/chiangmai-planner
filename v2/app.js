...
function gmaps(p){
  ...
  if(p.placeId) return 'https://www.google.com/maps/place/?q=place_id:'+encodeURIComponent(p.placeId);
  const term=(p.en&&/[a-zA-Z]/.test(p.en))?p.en:p.name;            // 既有座標搜尋 fallback
...
function renderMarkers(){
  ...
  spotIds=null; spotFromPicker=false; // 任何重繪/篩選清除 spotlight 狀態（新 marker 為乾淨樣式）
  ...
  if(pendingSpot){ const s=pendingSpot; pendingSpot=null; applySpot(s); }   // 僅套用「剛被 mapSpotlight* 觸發開啟抽屜」的待辦（消費一次；一般篩選重繪不套用＝清除）
}
...
function applySpot(ids){
  ...
  // 先移動地圖（animate:false 同步寫定 Leaflet 的 translate3d），再套樣式：否則 setView 重排會覆寫剛附上的 scale → 放大消失
  if(any&&bounds.length){ if(bounds.length===1) map.setView(bounds[0],16,{animate:false}); else map.fitBounds(bounds,{padding:[40,40],maxZoom:15,animate:false}); }
  markers.forEach(m=>{ const el=m.getElement&&m.getElement(); if(!el) return;
    const hit=set.has(m._pid);
    el.style.opacity=hit?'1':'0.25';
    el.style.transform=el.style.transform.replace(/\s*scale\([^)]*\)/,'')+(hit?' scale(1.25)':'');
...
function openSheet(html){ sh.classList.remove('sheet-edit','sheet-picker','pk2','tall'); ['pkDay','pkSlot','pkMode','qaPid','qaMove','omPid','omMove','omDay','omSlot','opEid'].forEach(k=>delete sh.dataset[k]); sh.innerHTML='<button class="sheetclose" data-action="close">關閉</button>'+html; ov.classList.add('on'); document.body.classList.add('sheet-open'); }
function closeSheet(){
